...
/* ---- helpers to show/hide modals (inline style beats CSS classes) ---- */
...
function initDrop(){
  ...
  document.addEventListener('dragenter',function(e){
    ...
    if(mc2&&mc2.contains(e.target)){dragCount++;overlay.style.display='flex';}
  },true);
  document.addEventListener('dragleave',function(e){
    var mc2=document.querySelector('.map-container');
    if(mc2&&mc2.contains(e.target)){dragCount--;if(dragCount<=0){dragCount=0;overlay.style.display='none';}}
  },true);
  ...
  document.addEventListener('drop',function(e){
    ...
    var f=getCADFile(e);
    if(!f){console.log('[CAD] dropped file is not DXF/DWG');return;}
    var ext=f.name.split('.').pop().toLowerCase();
    S.file=f;
    console.log('[CAD] file dropped:',f.name,ext);
    if(ext==='dxf'){
      document.getElementById('dxfDropModalFilename').textContent=f.name;
      showModal('dxfDropModal');
    } else if(ext==='dwg'){
      document.getElementById('dwgDropModalFilename').textContent=f.name;
      showModal('dwgDropModal');
    }
  },true);
}
...
/* ---- Save traced polygons via edge function ---- */
...
function initDXFModal(){
  var el=document.getElementById('dxfDropModal');if(!el)return;
  document.getElementById('dxfDropModalClose').addEventListener('click',function(){hideModal('dxfDropModal');});
  el.addEventListener('click',function(e){if(e.target===el)hideModal('dxfDropModal');});
  document.getElementById('dxfDropPreviewBtn').addEventListener('click',function(){
    ...
    hideModal('dxfDropModal');readAct(S.file,crs,'preview');});
  document.getElementById('dxfDropUpdateBtn').addEventListener('click',function(){
    var crs=document.getElementById('dxfDropCrsSelect').value;if(!crs){alert('Select a coordinate system.');return;}
    hideModal('dxfDropModal');readAct(S.file,crs,'update');});
  document.getElementById('dxfDropInspectBtn').addEventListener('click',function(){hideModal('dxfDropModal');openCAD(S.file.name,S.file);});
}
/* ---- Wire DWG drop modal ---- */
function initDWGModal(){
  var el=document.getElementById('dwgDropModal');if(!el)return;
  document.getElementById('dwgDropModalClose').addEventListener('click',function(){hideModal('dwgDropModal');});
  el.addEventListener('click',function(e){if(e.target===el)hideModal('dwgDropModal');});
  document.getElementById('dwgDropOpenBtn').addEventListener('click',function(){hideModal('dwgDropModal');openCAD(S.file.name,S.file);});
}
/* ---- Wire CAD Inspector close ---- */
function initCADClose(){
  var b=document.getElementById('cadInspectorClose');
  if(b)b.addEventListener('click',function(){
    var p=document.getElementById('cadInspectorPanel');if(p)p.style.display='none';
    var f=document.getElementById('cadInspectorFrame');if(f)f.src='about:blank';
  });
}
...
function initToolbar(){
  ...
  if(traceBtn)traceBtn.addEventListener('click',function(){
    ...
    if(!S.tracedSource){S.tracedSource=new ol.source.Vector();S.tracedLayer=new ol.layer.Vector({source:S.tracedSource,style:new ol.style.Style({stroke:new ol.style.Stroke({color:'#facc15',width:2.5}),fill:new ol.style.Fill({color:'rgba(250,204,21,0.06)'})}),zIndex:1000});m.addLayer(S.tracedLayer);}
  ...
  if(saveBtn)saveBtn.addEventListener('click',function(){var r=document.querySelector('input[name="polygonLayer"]:checked');if(!r){setStatus('Select a target layer first.','error');return;}saveTraced(r.value);});
...
function initPanel(){
  ...
  if(da){da.addEventListener('dragover',function(e){e.preventDefault();da.style.borderColor='#818cf8';});da.addEventListener('dragleave',function(){da.style.borderColor='#6366f1';});da.addEventListener('drop',function(e){e.preventDefault();da.style.borderColor='#6366f1';if(e.dataTransfer.files[0])handleFile(e.dataTransfer.files[0]);});}
...
function handleFile(f){if(!f)return;if(f.name.split('.').pop().toLowerCase()!=='dxf'){setStatus('Only .dxf files supported here.','error');return;}S.file=f;setFn(f.name);setStatus('Reading\u2026','');var crsEl=document.getElementById('polygon-crs-confirm'),crs=(crsEl&&crsEl.value)||'EPSG:32636';var r=new FileReader();r.onload=function(ev){S.text=ev.target.result;S.crs=crs;renderDXF(S.text,crs,f.name);};r.readAsText(f);}

function readAct(f,crs,act){var r=new FileReader();r.onload=function(ev){S.text=ev.target.result;S.crs=crs;if(act==='preview')renderDXF(S.text,crs,f.name);else if(act==='update'){renderDXF(S.text,crs,f.name);setTimeout(function(){var dt=document.getElementById('importSubTabDxf');if(dt)dt.click();},400);}else if(act==='inspect')openCAD(f.name,f);};r.readAsText(f);}

window.openCadInspector=openCAD;

function init(){regProj();initDrop();initDXFModal();initDWGModal();initCADClose();initBadge();initToolbar();initPanel();console.log('[GSPNET CAD] v3 ready');}
